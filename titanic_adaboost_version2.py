# -*- coding: utf-8 -*-
"""
Created on Fri Dec  8 12:12:17 2017
...
ada = AdaBoostClassifier(algorithm='SAMME.R', base_estimator=None,
          learning_rate=0.8, n_estimators=6, random_state=None)
pa={ 'n_estimators':list(range(3,100,3))}
ada=GridSearchCV(ada,pa)
ada.fit(X_train,y_train)
ada=ada.best_estimator_
ada

pa={ 'learning_rate':[0.1,0.3,0.5,0.6,0.8]}
ada=GridSearchCV(ada,pa)
ada.fit(X_train,y_train)
ada=ada.best_estimator_
ada
...
#0.81
ada.fit(X_train, y_train)
print("ada: {0:.2}".format(ada.score(X_test, y_test)))
