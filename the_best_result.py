...
et_oof_train=pd.read_csv("D:/data_file/kaggle_valid_result/et_train_valid.csv").values
et_oof_test=pd.read_csv("D:/data_file/kaggle_valid_result/et_test_valid.csv").values
...
model_merge_table=pd.DataFrame({'svc':svc_oof_train.ravel(),'kn':kn_oof_train.ravel(),'forest':forest_oof_train.ravel(),'ada':ada_oof_train.ravel(),\
              'bag':bag_oof_train.ravel(),'dt':dt_oof_train.ravel(),'et':et_oof_train.ravel()})
...
model=xgb.XGBClassifier(base_score=0.5, colsample_bylevel=1, colsample_bytree=0.82,
       gamma=0.91, learning_rate=0.09, max_delta_step=0, max_depth=2,
       min_child_weight=0.008, missing=None, n_estimators=7, nthread=-1,
...
test_df=pd.DataFrame({'svc':svc_oof_test.ravel(),'forest':forest_oof_test.ravel(),'ada':ada_oof_test.ravel(),\
              'bag':bag_oof_test.ravel(),'dt':dt_oof_test.ravel(),'et':et_oof_test.ravel()}) 
