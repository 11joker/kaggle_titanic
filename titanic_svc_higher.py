...
for df in train, test:
    df['Shared_ticket'] = np.where(df.\
           groupby('Ticket')['Name'].transform('count') > 1, 1, 0)
...
nominal_features = ["Pclass", "Sex", "Embarked", "FamilySize", "Title","Shared_ticket"]
for df in train, test:
    for nominal in nominal_features:
        df[nominal] = df[nominal].astype(dtype="category")
        
feature=["Pclass", "Sex", "Age", "SibSp", "Parch",
              "Fare", "Embarked", "FamilySize", "Title", "Shared_ticket"]
...
svc = svm.SVC(C=0.8, cache_size=200, class_weight='balanced', coef0=0.0,
  decision_function_shape=None, degree=3, gamma='auto', kernel='rbf',
  max_iter=-1, probability=False, random_state=None, shrinking=True,
  tol=0.001, verbose=False)
pa={ 'C':[0.6,0.5,0.7,0.8]}
...
prediction = svc.predict(test[predictors])
